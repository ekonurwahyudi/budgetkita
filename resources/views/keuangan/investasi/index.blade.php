...
<div class="grid w-full space-y-5">
    <div class="kt-card">
        <div class="kt-card-header min-h-16">
            <input type="text" placeholder="Cari..." class="kt-input" style="width:200px" data-kt-datatable-search="#investasi_table" />
            @can('investasi.create')
            <a href="{{ route('investasi.create') }}" class="kt-btn kt-btn-outline">
                ...
                <table class="kt-table" data-kt-datatable-table="true">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $item->kategoriInvestasi?->deskripsi ?? '-' }}</td>
                            ...
                            <td class="text-end">
                                <span class="inline-flex gap-2.5">
                                    ...
                                    <form method="POST" action="{{ route('investasi.approve', $item) }}" class="inline">@csrf<button type="submit" class="kt-btn kt-btn-primary kt-btn-sm kt-btn-icon" title="Approve"><i class="ki-filled ki-check"></i></button></form>
                                    <form method="POST" action="{{ route('investasi.reject', $item) }}" class="inline" onsubmit="return confirm('Yakin reject?')">@csrf<button type="submit" class="kt-btn kt-btn-destructive kt-btn-sm kt-btn-icon" title="Reject"><i class="ki-filled ki-cross"></i></button></form>
                                    @endif
                                    <a href="{{ route('investasi.show', $item) }}" class="kt-btn kt-btn-sm kt-btn-icon kt-btn-outline" title="Lihat"><i class="ki-filled ki-eye"></i></a>
                                    @can('investasi.edit')@if(auth()->user()->hasRole('Owner') || in_array($item->status, ['awaiting_approval','pending']))
                                    <a href="{{ route('investasi.edit', $item) }}" class="kt-btn kt-btn-sm kt-btn-icon kt-btn-outline" title="Edit"><i class="ki-filled ki-pencil"></i></a>
                                    @endif @endcan
                                    @can('investasi.delete')@if(auth()->user()->hasRole('Owner') || $item->status === 'awaiting_approval')
