...
@section('page-description', 'Kelola data kategori transaksi')

@section('content')
<div class="grid w-full space-y-5">
    <div class="kt-card">
        <div class="kt-card-header min-h-16">
            <input type="text" placeholder="Cari..." class="kt-input" style="width:200px" data-kt-datatable-search="#kategori_transaksi_table" />
            @can('kategori-transaksi.create')
            <button type="button" class="kt-btn kt-btn-outline" onclick="openCreateModal()">
                <i class="ki-filled ki-plus-squared"></i> Tambah
            </button>
            @endcan
        </div>
        <div id="kategori_transaksi_table" class="kt-card-table" data-kt-datatable="true" data-kt-datatable-page-size="10" data-kt-datatable-state-save="false" data-kt-datatable-state-namespace="kategori_transaksi">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col" data-kt-datatable-column="kode">
                                <span class="kt-table-col">
                                    <span class="kt-table-col-label">Kode Kategori</span>
                                    <span class="kt-table-col-sort"></span>
                                </span>
                            </th>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <span class="inline-flex gap-2.5">
                                    @can('kategori-transaksi.edit')
                                    <button class="kt-btn kt-btn-sm kt-btn-icon kt-btn-outline" onclick="openEditModal('{{ $item->id }}')">
                                        <i class="ki-filled ki-pencil"></i>
                                    </button>
                                    @endcan
                                    @can('kategori-transaksi.delete')
                                    <form method="POST" action="{{ route('kategori-transaksi.destroy', $item) }}" onsubmit="return confirm('Yakin hapus data ini?')">
                                        @csrf @method('DELETE')
...
<div class="kt-modal" data-kt-modal="true" id="formModal">
    <div class="kt-modal-content max-w-[500px] top-5 lg:top-[15%]">
        <div class="kt-modal-header">
            <h3 class="kt-modal-title" id="modalTitle">Tambah Kategori Transaksi</h3>
        ...
        <form id="dataForm" method="POST">
            ...
            <div class="kt-modal-body flex flex-col gap-4">
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-foreground">Kode Kategori <span class="text-danger">*</span></label>
                    <input type="text" name="kode_kategori" id="kode_kategori" class="kt-input" required>
                </div>
                <div class="flex flex-col gap-1">
...
function openCreateModal() {
    document.getElementById('modalTitle').textContent = 'Tambah Kategori Transaksi';
    document.getElementById('dataForm').action = "{{ route('kategori-transaksi.store') }}";
...
function openEditModal(id) {
    fetch(`/masterdata/kategori-transaksi/${id}/edit`)
        .then(r => r.json())
        .then(data => {
            document.getElementById('modalTitle').textContent = 'Edit Kategori Transaksi';
            document.getElementById('dataForm').action = `/masterdata/kategori-transaksi/${id}`;
