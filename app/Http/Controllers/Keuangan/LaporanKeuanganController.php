...
class LaporanKeuanganController extends Controller
{
    ...
    private function getQueryData(Request $request)
    {
        ...
        if ($jenis === 'semua' || $jenis === 'uang_keluar') {
            $q = PembelianPersediaan::with(['accountBank'])->withSum('items as total_nominal', 'harga_total');
            if ($status) $q->where('status', $status);
            if ($tgl_dari) $q->whereDate('tgl_pembelian', '>=', $tgl_dari);
            if ($tgl_sampai) $q->whereDate('tgl_pembelian', '<=', $tgl_sampai);
            foreach ($q->latest()->get() as $p) {
                $rows->push([
                    ...
                    'siklus' => null,
                    'kategori' => 'Persediaan',
                    'nominal' => (float) ($p->total_nominal ?? 0),
                    'status' => $p->status,
        ...
        // Pembelian Aset
        if ($jenis === 'semua' || $jenis === 'uang_keluar') {
            $q = PembelianAset::with(['kategoriAset', 'accountBank']);
            if ($status) $q->where('status', $status);
            if ($tgl_dari) $q->whereDate('tgl_pembelian', '>=', $tgl_dari);
            if ($tgl_sampai) $q->whereDate('tgl_pembelian', '<=', $tgl_sampai);
            if ($search) $q->where('nama_aset', 'like', "%{$search}%");
            foreach ($q->latest()->get() as $a) {
                $rows->push([
                    'type' => 'Pembelian Aset',
                    ...
                    'nominal' => (float) $a->nominal_pembelian,
                    'status' => $a->status,
        ...
        $totalMasuk = $rows->where('jenis_raw', 'uang_masuk')->sum('nominal');
        $totalKeluar = $rows->where('jenis_raw', 'uang_keluar')->sum('nominal');
