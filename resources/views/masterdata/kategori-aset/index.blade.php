...
@section('page-description', 'Kelola data kategori aset')

@section('content')
<div class="grid w-full space-y-5">
    <div class="kt-card">
        <div class="kt-card-header min-h-16">
            <input type="text" placeholder="Cari..." class="kt-input" style="width:200px" data-kt-datatable-search="#kategori_aset_table" />
            @can('kategori-aset.create')
            <button type="button" class="kt-btn kt-btn-outline" onclick="openCreateModal()">
                <i class="ki-filled ki-plus-squared"></i> Tambah
            </button>
            @endcan
        </div>
        <div id="kategori_aset_table" class="kt-card-table" data-kt-datatable="true" data-kt-datatable-page-size="10" data-kt-datatable-state-save="false" data-kt-datatable-state-namespace="kategori_aset">
                    ...
                    <thead>
                        <tr>
                            ...
                            <th scope="col" data-kt-datatable-column="kode">
                                <span class="kt-table-col"><span class="kt-table-col-label">Kode Aset</span><span class="kt-table-col-sort"></span></span>
                            </th>
                        ...
                        <tr>
                            ...
                            <td class="text-end">
                                <span class="inline-flex gap-2.5">
                                    @can('kategori-aset.edit')
                                    <button class="kt-btn kt-btn-sm kt-btn-icon kt-btn-outline" onclick="openEditModal('{{ $item->id }}')">
                                        <i class="ki-filled ki-pencil"></i>
                                    </button>
                                    @endcan

                                    @can('kategori-aset.delete')
                                    <form method="POST" action="{{ route('kategori-aset.destroy', $item) }}" onsubmit="return confirm('Yakin hapus data ini?')">
                                        @csrf @method('DELETE')
...
<div class="kt-modal" data-kt-modal="true" id="formModal">
    <div class="kt-modal-content max-w-[500px] top-5 lg:top-[15%]">
        <div class="kt-modal-header">
            <h3 class="kt-modal-title" id="modalTitle">Tambah Kategori Aset</h3>
        ...
        <form id="dataForm" method="POST">
            ...
            <div class="kt-modal-body flex flex-col gap-4">
                <div class="flex flex-col gap-1">
                    <label class="text-sm font-medium text-foreground">Kode Aset <span class="text-danger">*</span></label>
                    <input type="text" name="kode_aset" id="kode_aset" class="kt-input" required>
                </div>
                <div class="flex flex-col gap-1">
...
function openCreateModal() {
    ...
    document.getElementById('dataForm').action = "{{ route('kategori-aset.store') }}";
...
function openEditModal(id) {
    fetch(`/masterdata/kategori-aset/${id}/edit`)
        .then(r => r.json())
        .then(data => {
            document.getElementById('modalTitle').textContent = 'Edit Kategori Aset';
            document.getElementById('dataForm').action = `/masterdata/kategori-aset/${id}`;
